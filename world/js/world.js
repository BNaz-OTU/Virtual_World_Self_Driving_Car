class World {
  ...
  // building is located or the highest tip of a road (whichevr one is near the
  // top). Same thing is done for 'left', 'right', and 'bottom' with their
  ...
  #generateTrees() {
    ...
    while (tryCount < 100) {
      ...
      // Checks if trees are in illegal spots (within or near buildings/roads)
      let keep = true;
      ...
      // Checks to see if the tree is close enough to a nearby object so its
      // worth printing (trees not placed in the middle of nowhere)
      if (keep) {
  ...
  #generateBuildings() {
    // 'tmpEnvelopes' creates an overlapping 'transparent road' over the regular
    // road, this is to help show much space is needed to build the buildings.
    // This will be influential in the other steps because they well only extract
    // the outer lines of the envelope and that piece will be considered the
    // buildable area
    const tmpEnvelopes = [];
    for (const seg of this.graph.segments) {
      tmpEnvelopes.push(
    ...
    // Using 'tmpEnvelopes', 'guides' creates a long line showing how far off the
    // road the buildable area is, it eliminates most of the enveloping polygon to
    // demonstrate this buidable area
    ...
    // Parsing through "guides" further, "supports" Creates the buffer zone/space
    // needed demonstrating how much space is needed build the building (prints
    // dashed lines representing the suitable areas for a building)
    const supports = [];
    ...
    // From using 'supports', bases creates the foundation/envelope of the building (prints rectangles)
    const bases = [];
    for (const seg of supports) {
      bases.push(new Envelope(seg, this.buildingWidth).poly);
    }

    // With the help of 'bases', this loop will help find bases that are
    // overlapping with each other (possible when roads are at certain angles/
    // close to each other),
    const eps = 0.001;
